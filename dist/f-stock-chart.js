...
var StockChart;
(function (StockChart) {
    var NetWorthLine = (function (_super) {
        __extends(NetWorthLine, _super);
        function NetWorthLine(options) {
            _super.call(this, options);
            this.dateline = options.dateline;
        }
        NetWorthLine.prototype.getPointsCount=function(){
            return 5*240;
        };
        NetWorthLine.prototype.popWindowDatas = function (index) {
            var _a = this,prices = _a.prices, times = _a.times, preClosePrice=_a.preClosePrice;
            var calcPercent = function (price) {
                return ((price - preClosePrice) / preClosePrice * 100).toFixed(2) + '%';
            };

            return [{color: _a.popWindowValueColor, value: times[index]},
                {color: _a.popWindowValueColor, value: prices[index]}];
        };
        NetWorthLine.prototype.drawAxisText = function () {
            ...
            var _b = this,times=_b.times, roofPrice = _b.roofPrice, floorPrice = _b.floorPrice, preClosePrice = _b.preClosePrice, roofPercent = _b.roofPercent, floorPercent = _b.floorPercent;
            ...
            for(var i=0;i<5;i++){
                var index = (figureWidth / 4)*i/unitX;
                if(i==0){
                    ctx.textAlign = "left";
                }else if(i==4){
                    ctx.textAlign = "right";
                ...
                if(times[index]){
                    this.drawText(times[index], [(figureWidth / 4)*i, axisY]);
                }
            }
        ...
        NetWorthLine.prototype.drawVolumeLine = function () {
        };
        return NetWorthLine;
    }(StockChart.ITrendLine));

    function drawNetWorthLine(options) {
        var defaultOptions = {
            grid: {
                x: 4,
                y: 4,
            ...
            avgLineColor: 'rgba(169,77,180,.5)',
            popWindowValueColor: 'rgba(0,0,0,1)',
            popWindowKeys: {color: '#939393', keys: ['日期:', '净值:']}
        };
        options = StockChart.mixins({}, defaultOptions, options);
        var netWorthLine = new NetWorthLine(options);
        netWorthLine.initialize();
    }

    StockChart.drawNetWorthLine = drawNetWorthLine;
})(StockChart || (StockChart = {}));